# -*- coding: utf-8 -*-
"""
Created on Wed Aug 11 15:20:26 2021
...
def file_name(file_dir):
    ...
    return files
files = file_name('./csi100_rawdata')
files.remove('600745.csv')
print('600745.csv' in files)
calib_info = pd.read_csv('./csi100_rawdata/600519.csv')
calib_date = [str(d)[0:4]+'-'+str(d)[4:6]+'-'+str(d)[6:] for d in calib_info['trade_date']]
calib_date.reverse()

num_delet = 0
for f in files:
    df = pd.read_csv('./csi100_rawdata/'+f)
    if len(df) < 71 or df['trade_date'][len(df)-1] != 20131231:
        ...
        df.to_csv('./csi100_calibdata/'+f,index=False)
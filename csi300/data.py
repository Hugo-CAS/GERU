# -*- coding: utf-8 -*-
"""
Created on Wed Aug 11 16:30:08 2021
...
def file_name(file_dir):
    ...
    return files
files = file_name('./csi300_processeddata')
...
for f in files:
#    print(f)
    df = pd.read_csv('./csi300_processeddata/'+f)
    return_list = np.log(df[(df['trade_date']>'2015-01-01')&(df['trade_date']<'2021-01-01')]['pct_chg']+1)
    return_list_3 = np.log(df[(df['trade_date']>'2015-03-01') & (df['trade_date']<'2021-05-01')]['pct_chg_3']+1)
    ...
    price_matrix[f.replace('.csv','')] =  np.log(df['pct_chg']+1)
...
#            if j==42:
#                print(adj_temp[j])
#                print(abs(adj_temp[j,:]).sum())
            if int(abs(adj_temp[j,:]).sum()) == 1:
                print(j)
                topk_index = abs(corr_matirx[i,j,:]).argsort()[-topk:-2]
#                if j == 137:
#                    print(corr_matirx[i,j,topk_index],topk_index)
                if corr_matirx[i,j,topk_index] > 0:
                    adj_temp[j,topk_index] = 1
                else:
                    adj_temp[j,topk_index] = -1
    # adj_temp_t = adj_temp.copy()
    # adj_temp = adj_temp + adj_temp.T
    # Adj_t.append(adj_temp)
...
np.save('./csi300_graphdata/Adj.npy', abs(Adj))
np.save('./csi300_graphdata/Adj_sign.npy', Adj_t)
np.save('./csi300_graphdata/label_matrix_up_down.npy',label_matrix)
np.save('./csi300_graphdata/label_matrix_up_down_3.npy',label_matrix_3)
np.save('./csi300_graphdata/label_matrix_up_down_6.npy',label_matrix_6)
...
for i in range(len(label_matrix)):
    ...
    feature_matrix = []
    for f in files:
        df = pd.read_csv('./csi300_processeddata_1/'+f)
        feature_matrix.append(df.iloc[i+1:i+13][['close','open','high','low','pct_chg','vol','amount']].values)
    feature_matrix = np.array(feature_matrix)
    np.save('./csi300_graphdata/'+str(df['trade_trade_date'].iloc[i+13])+'.npy',feature_matrix)
